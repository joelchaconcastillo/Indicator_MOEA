...
void CMOEAD::evol_population()
{
    ...
    for(int sub=0; sub<pops; sub++)
	{
		...
		diff_evo_xoverA(population[sub].indiv,population[plist[0]].indiv,population[plist[1]].indiv, population[plist[2]].indiv, child, rate2);
...
void CMOEAD::exec_emo(int run)
{
	...
	sprintf(filename1,"%s/POS/POS_MOEAD_%s_RUN%d_seed_%d_nobj_%d_nvar_%d_DI_%lf_DF_%lf",strpath, strTestInstance,run, seed, nobj, nvar, Di/sqrt(nvar), Df);
	sprintf(filename2,"%s/POF/POF_MOEAD_%s_RUN%d_seed_%d_nobj_%d_nvar_%d_DI_%lf_DF_%lf",strpath, strTestInstance,run, seed, nobj, nvar, Di/sqrt(nvar), Df);
