...
int CMOEAD::max_HV_contribution(unordered_set<int> &candidates, unordered_set<int> &candidates_front, unordered_set<int> &survivors, unordered_set<int> &survivors_front)
{
  ...
  vector<vector<double> > all(survivors_front.size()+candidates_front.size());
  int size=0;
  vector<double> ref(nobj, -DBL_MAX);
  vector<int> idx_rev(candidates_front.size());
  ...
  vector<pair<double, size_t> > res=m_indicator.largestContributors(all, (size_t)all.size());
  pair<double, int> max_ctr(-DBL_MAX, -1);
  int maxid=-1;
  for(auto pp:res){
	if(pp.second >= survivors_front.size()){
	   maxid= idx_rev[pp.second-(int)survivors_front.size()]; break;
	}
  }
  survivors_front.insert(maxid);
  survivors.insert(maxid);
  candidates.erase(maxid);
  candidates_front.erase(maxid);
  return maxid;
}
#endif
