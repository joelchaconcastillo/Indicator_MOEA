...
class CMOEAD
{
   ...
   public:
	...
	void idxs_dominance_information(vector<int> &idxs);
...
void CMOEAD::init_population()
{
    ...
    for(int i=0; i< nPop+nOffspring; i++)
    {
     ...
     idxs_dominance_information(parent_idx); //it is necessary for the binary tournament, it can be faster testing.....
}
void CMOEAD::evol_population()
{
   for(int i = 0; i < nOffspring; )
   {
      ...
      else if(Rp[parent_idx[idx1]] == Rp[parent_idx[idx2]] && rand()%2) idx1=idx2;

      if(Rp[parent_idx[idx3]] > Rp[parent_idx[idx4]]) idx3=idx4;
      else if(Rp[parent_idx[idx3]] == Rp[parent_idx[idx4]] && rand()%2) idx3=idx4;
      // produce a child solution
      CIndividual child1 = pool[parent_idx[idx1]], child2 = pool[parent_idx[idx3]];
      bool crosed = real_sbx_xoverA(pool[parent_idx[idx1]], pool[parent_idx[idx3]], child1, child2);
      if(rand()%2) swap(child1, child2);
...
//  m_indicator.setReference(ref);
  ...
  for(auto idx:candidates_front)
  {
     ...
     max_ctr = max(max_ctr, make_pair(res[0].first, idx));
  }
...
void CMOEAD::idxs_dominance_information(vector<int> &points)
{
   int n=points.size();
   ...
   for(int pidx1=0; pidx1< n; pidx1++)
   {
      ...
      {
	if(pidx1 == pidx2) continue;
	DominanceRelation ref = dominance(pool[points[pidx1]].y_obj, pool[points[pidx2]].y_obj);
        if(ref==LHS_DOMINATES_RHS) Sp[pidx1].insert(pidx2);
 	else if(ref==RHS_DOMINATES_LHS) Np[pidx1]++;
//        if( pool[points[pidx1]] < pool[points[pidx2]]) Sp[pidx1].insert(pidx2);
// 	else if( pool[points[pidx2]] < pool[points[pidx1]]) Np[pidx1]++;

      }
      if( Np[pidx1] == 0)
      {
         fronts[rank].push_back(pidx1);
	 Rp[points[pidx1]]=rank;
      }
   }
   while(true)
   {
	...
	for(auto idx_dominated:Sp[idx])
	  ...
	  Np[idx_dominated]--;
          ...
          {
	     next_front.push_back(idx_dominated);
	     Rp[points[idx_dominated]] = rank+1;
          }
        }
      }
      if(next_front.empty()) break;
