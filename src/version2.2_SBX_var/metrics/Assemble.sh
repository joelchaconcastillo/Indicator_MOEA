...
do
  ...
  do
     for sed in {1..35}
     do
     tail -100 $PATH1/POF_*_${instance}_RUN${sed}_seed_*nobj_2_*_DI_${DI}*CR_${px}*_F_*0.75* | cut -f1,3 -d' '  > POF/${instance}_2_${sed}_${px}
     done
  done
  ...
  do
     for sed in {1..35}
     do
     tail -100 $PATH1/POF_*_${instance}_RUN${sed}_seed_*nobj_3*_DI_${DI}*CR_${px}*_F_*0.75* | cut -f1,3,5 -d' '  > POF/${instance}_3_${sed}_${px}
     done
  done
done
